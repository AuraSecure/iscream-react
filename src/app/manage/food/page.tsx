...
export default function ManageFoodPage() {
  const { data, setData, loading, saving, msg, setMsg, hasChanges, save, handleDiscard } =
    useContentManager<FoodData>({
      apiPath: "/api/content/food",
    });
  ...
  if (loading) return <main style={{ padding: 24 }}>Loading food menu…</main>;
  if (!data) return <main style={{ padding: 24 }}>Could not load food menu data.</main>;

  return (
    <section className="bg-gray-100 p-6 rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Manage Food Menu</h1>
        <button
          type="button"
